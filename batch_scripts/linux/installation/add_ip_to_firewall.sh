...
# pytca webapp and also so MongoDB can be accessed
...
# for MongoDB access (only add IP for systems where we are running pytca) - server_port 27017
...
# we are assuming that Redis is running on the pytca machine
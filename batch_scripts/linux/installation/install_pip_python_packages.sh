...
# Installs all the Python packages needed for pytca, it also installs some additional packages for Jupyter, so you
# can interact with pytca with Jupyter (strictly speaking if we could ignore these, if you don't wish to run Jupyter on
# top of pytca
...
if [ $pytca_PYTHON_ENV_TYPE == "virtualenv" ]; then
    ...
    pip install \
        ...
        IPython==7.14.0 chartpy==0.1.8 findatapy==0.1.19 dash-auth==1.3.2 cufflinks==0.17.3 plotly==4.14.3 kaleido \
...
elif [ $pytca_PYTHON_ENV_TYPE == "conda" ] && [ $CONDA_FROM_YAML == 0 ]; then
    ...
    pip install arctic==1.79.4 mysql-connector-python==8.0.19 chartpy==0.1.8 findatapy==0.1.19 dash-auth==1.3.2 cufflinks==0.17.3 plotly==4.14.3 kaleido \

...
# This will install necessary components for pytca. In practice, you might end up having different configuations, because
...
# - install various Python packages necessary for pytca into the Python virtual environment
# - install nginx web server (which is the recommended server for pytca's web app)
# - install pytca on nginx, so the pytca web app is accessable
...
# - install pytca on Apache, which is deprecated
...
# Install nginx web server (primary web server supported by pytca)
...
# Setup the pytca application so that it can be picked up nginx/gunicorn
source install_pytca_on_nginx_gunicorn.sh
# source install_pytca_on_apache_gunicorn.sh
# source install_pytca_on_apache.sh # uses WSGI, but this tends to be slower

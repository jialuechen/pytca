...
# Will restart the databases/key-value stores used by pytca
...
if [ $START_MONGODB == 1 ]; then
  ...
  sudo mongod --config $pytca_CUEMACRO/pytca/conf/mongo.conf
...
sudo redis-server $pytca_CUEMACRO/pytca/conf/redis.conf --daemonize yes

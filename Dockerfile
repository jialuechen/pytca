...
COPY requirements.txt /tmp/pytca/requirements.txt
...
RUN buildDeps='gcc g++ libsnappy-dev unixodbc-dev build-essential libcairo2 libpango-1.0-0 libpangocairo-1.0-0 libgdk-pixbuf2.0-0 libffi-dev shared-mime-info' && \
    ...
    pip install --no-cache-dir -r /tmp/pytca/requirements.txt && \
    rm  /tmp/pytca/requirements.txt
    # && \
    #apt-get purge -y --auto-remove $buildDeps

# Copy to /
COPY ./pytca /pytca/pytca
COPY ./pytcagen /pytca/pytcagen
COPY ./pytcauser /pytca/pytcauser
COPY ./test /pytca/test
COPY ./test /test

# Make sure pytca on the PYTHONPATH
ENV PYTHONPATH "${PYTHONPATH}:/pytca"
...
CMD if [ "${RUN_PART}" = 1 ]; \
    then py.test --cov=pytca  --cov-report html:artifacts/html-coverage --cov-report term --html=artifacts/html-report/report.html --ignore-glob='*multithreading*.py'; \
    else py.test --cov=pytca  --cov-report html:artifacts/html-coverage --cov-report term \
        --html=artifacts/html-report/report.html; \
    fi

# Run everything
# CMD py.test --cov=pytca  --cov-report html:artifacts/html-coverage --cov-report term \
#        --html=artifacts/html-report/report.html

# Example to run a specific test script
# CMD py.test --cov=pytca  --cov-report html:artifacts/html-coverage --cov-report term \
#    --html=artifacts/html-report/report.html test/test_pytca/test_tca_multithreading.py

# Example to run an individual test function
# CMD py.test --cov=pytca  --cov-report html:artifacts/html-coverage --cov-report term \
#    --html=artifacts/html-report/report.html test/test_pytca/test_data_read_write.py::test_write_trade_data_sql

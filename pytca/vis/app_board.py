...
from pytca.vis.app_imports import *
...
if debug_start_flask_server_directly:
    ...
    url_prefix = 'pytcaboard' # eg. if hosted on "http://localhost/pytcaboard" (can also be empty)
    # url_prefix = constants.url_prefix # eg. if hosted on "http://localhost/pytca" (can also be empty)

stylesheets = ['pytca.css']
...
app = dash.Dash(name='pytcaboard', server=server, suppress_callback_exceptions=True, serve_locally=True, external_stylesheets=stylesheets_path)
app.title = 'pytcaboard'
app.server.secret_key = constants.secret_key

# allow pytca to be hosted on a different url eg. not on root
if debug_start_flask_server_directly:
    ...
    app.config.update({
        'routes_pathname_prefix': '/pytcaboard/',
        'requests_pathname_prefix': '/pytcaboard/'
    })
...
if constants.pytca_version == 'user':
    try:
        ...
        from pytcauser.layoutboarduser import *;

        layout = LayoutImplBoardUser(app=app, constants=constants, url_prefix=url_prefix)
        from pytcauser.tcacallerboarduser import TCACallerImplBoardUser as TCACaller
    except:
        from pytcagen.layoutboardgen import *;

        layout = LayoutDashImplBoardGen(app=app, constants=constants, url_prefix=url_prefix)
        from pytcagen.tcacallerboardgen import TCACallerImplBoardGen as TCACaller

# this loads up a generic version of the layout and TCA application
elif constants.pytca_version == 'test_pytca' or constants.pytca_version == 'gen':
    from pytcagen.layoutboardgen import *;
...
plain_css = open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pytca.css'), 'r').read()
...
from pytca.vis.displaylisteners import DisplayListeners
...
if __name__ == '__main__':
    ...
    from pytca.util.swim import Swim; Swim()

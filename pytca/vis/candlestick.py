"""Script for generating candlesticks in Plotly. This is a speed optimised version of the candlestick FigureFactory in the Plotly code
(which is very slow), which extensively uses NumPy to speed up creation of arrays which will later be used for plotting.
"""
...
## faster version of Plotly's candlestick class (uses NumPy methods)
...
class _Candlestick(object):
    ...
    def get_candle_increase(self):
        ...
        # use numpy, much quicker than plotly implementation using for loops
        np.warnings.filterwarnings('ignore')

        # x - time, y - price
        return self._assign_x_y_candlestick(self.close > self.open)
    ...
    def _assign_x_y_candlestick(self, index):

        try:
            y = np.array([self.low[index], self.open[index], self.close[index], self.close[index], self.close[index],
                          self.high[index]]).transpose().flatten().tolist()

            date_x = self.x[index]

            x = pd.DatetimeIndex(np.array([date_x, date_x, date_x, date_x, date_x, date_x]).transpose().flatten().tolist())

            # list of dates and values (Plotly expects it in this format to create candlesticks)
            return x, y
        except:
            return [], []
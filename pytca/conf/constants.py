...
class Constants(object):
    """The constants of pytca are stored here. They govern behavior such as: _tickers, settings, markout
    ...
    pytca_version = docker_var('$pytca_VERSION', 'gen')  # 'user' for user specific or 'gen' for generic version (also for future usage 'test_pytca')
    pytca_provider = docker_var('$pytca_PROVIDER', 'internal_pytca') # Will add external providers

    root_folder = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    log_folder = os.path.join(os.path.join(os.path.dirname(root_folder), 'log'))

    # Where is the test data held?
    test_data_folder = os.path.join(up(up(os.path.dirname(root_folder))), 'pytca_tests_data')

    temp_data_folder = os.path.join(os.path.join(up(up(os.path.dirname(root_folder))), 'pytca_tests_data'), 'temp2')
    ...
    ncfx_tickers = {'EURUSD': 'EURUSD',  # pytca ticker name vs. NCFX ticker name (should generally be the same)
                    'GBPUSD': 'GBPUSD',
                    'AUDUSD': 'AUDUSD',
                    'NZDUSD': 'NZDUSD',
                    'USDCAD': 'USDCAD',
    ...
    temp_data_folder = docker_var('/tmp/pytca/', '/tmp/pytca/')
    ...
    logging_file = os.path.join(log_folder, plat + "_pytca.log")
    ...
    ms_sql_server_username = docker_var('$MS_SQL_SERVER_USER', 'pytcauser')
    ms_sql_server_password = docker_var('$MS_SQL_SERVER_PASSWORD', 'pytcauser')
    ...
    mysql_username = docker_var("$MYSQL_USER", 'pytcauser')
    mysql_password = docker_var("$MYSQL_PASSWORD", 'pytcauser')
    ...
    pystore_data_store = 'pytca_store'
    ...
    # OVERWRITE_IN_ConstantsCred or set env var
    arctic_username = docker_var("$MONGO_INITDB_ROOT_USERNAME", 'pytcauser')
    arctic_password = docker_var("$MONGO_INITDB_ROOT_PASSWORD", 'pytcauser')
    ...
    # OVERWRITE_IN_ConstantsCred or set env var
    clickhouse_username = docker_var("$CLICKHOUSE_USERNAME", 'pytcauser')
    clickhouse_password = docker_var("$CLICKHOUSE_PASSWORD", 'pytcauser')
    ...
    volatile_cache_redis_internal_database = 1  # For storing pytca data (as opposed to for message broker reasons with Celery)
    ...
    # Using Redis as both can result in race conditions, because pytca extensively uses Redis to cache datasets
    ...
    url_prefix = 'pytca'  # eg. if hosted on "http://localhost/pytca"
    routes_pathname_prefix = '/pytca/'  # note: can sometimes be empty or simply '/'
    requests_pathname_prefix = '/pytca/'  # note: can sometimes be empty or simply '/'

    ##### OVERRIDES ########################################################################################################

    ### We can override default parameters by creating a separate constants file called constantscred.py in the pytca.conf package, below
    ...
    def __init__(self):
        ...
        # Which backend have you used 'user' or 'test_pytca'
        if Constants.pytca_version == 'user':
            # override fields related to assets etc. which are proprietary to each client
            try:
                from pytcagen.constantsgen import ConstantsGen
            ...
            # Override fields related to assets etc. which are proprietary to each client
            try:
                from pytcauser.constantsuser import ConstantsUser
        ...
        elif Constants.pytca_version == 'test_pytca':
            # Override fields related to assets etc. which are proprietary to each client
            try:
                from pytcagen.constantsgen import ConstantsGen
                cred_keys = ConstantsGen.__dict__.keys()
        ...
        elif Constants.pytca_version == 'gen':
            # Override fields related to assets etc. which are proprietary to each client
            try:
                from pytcagen.constantsgen import ConstantsGen
                cred_keys = ConstantsGen.__dict__.keys()
        ...
        try:
            from pytca.conf.constantscred import ConstantsCred
        ...
        try:
            from pytcauser.constantscreduser import ConstantsCredUser
